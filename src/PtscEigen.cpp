...
// -------------- Task ------------------
Task::Task(const Eigen::MatrixXd& t_A, const Eigen::VectorXd& t_b) : A(t_A), b(t_b)
{
  if (A.rows() != b.size())
...
{
  Eigen::VectorXd res =
      x.transpose() * A.transpose() * A * x + 2 * x.transpose() * A.transpose() * b + b.transpose() * b;
  return res(0);
}
...
uint32_t SVD::getRank() const
{
  return rank_;
}

// -------------- PTSC ------------------
PTSC::PTSC(const std::vector<Task>& tasks, const OsqpSettings& osqp_settings)
  : tasks_(tasks), problem_N_(tasks_[0].A.cols()), N_priorities_(tasks_.size()), osqp_settings_(osqp_settings)
{
  PTSC_type_ = UNCONSTRAINED;
}

PTSC::PTSC(const std::vector<Task>& tasks, const Eigen::VectorXd& lower_bounds, const Eigen::VectorXd& upper_bounds,
           const OsqpSettings& osqp_settings)
  : tasks_(tasks)
  , problem_N_(tasks_[0].A.cols())
...
PTSC::PTSC(const std::vector<Task>& tasks, const Eigen::MatrixXd& A_eq, const Eigen::VectorXd& b_eq,
           const Eigen::MatrixXd& A_ieq, const Eigen::VectorXd& b_ieq, const Eigen::VectorXd& lower_bounds,
           const Eigen::VectorXd& upper_bounds, const OsqpSettings& osqp_settings)
  : tasks_(tasks)
  , problem_N_(tasks_[0].A.cols())
  , N_priorities_(tasks_.size())
  , lower_bounds_(lower_bounds)
...
Eigen::VectorXd PTSC::solveConstrained()
{
  ...
  for (auto task : tasks_)
  {
    ...
    Eigen::VectorXd di = solveOnePriorityQP(Ai_dashed, bi_dashed, C_dashed, d_dashed, problem_feasible);
                                         ...
                                         bool& problem_feasible)
{
  ...
  if (PTSC_type_ == BOUND_CONSTRAINED)
  {
    ...
    qp_problem.A_ieq << C_dashed, -C_dashed;

    qp_problem.b_ieq = Eigen::VectorXd::Zero(d_dashed.rows() * 2);
    qp_problem.b_ieq << d_dashed - upper_bounds_, -d_dashed + lower_bounds_;
  }
  if (PTSC_type_ == FULLY_CONSTRAINED)
  {
    qp_problem.A_ieq = Eigen::MatrixXd::Zero(C_dashed.rows() * 2 + A_ieq_.rows(), C_dashed.cols());
    qp_problem.A_ieq << C_dashed, -C_dashed, A_ieq_ * C_dashed;
    qp_problem.b_ieq = Eigen::VectorXd::Zero(d_dashed.rows() * 2 + b_ieq_.rows());
    qp_problem.b_ieq << d_dashed - upper_bounds_, -d_dashed + lower_bounds_, b_ieq_ + A_ieq_ * d_dashed;
